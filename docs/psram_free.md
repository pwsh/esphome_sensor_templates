...
**Requires:** psram (core/psram.yaml)
...
packages:
  psram_free: !include
    file: esphome_sensor_templates/templates/diagnostics/psram_free.yaml
...
```yaml
packages:
  psram_free: github://OWNER/esphome_sensor_templates/templates/diagnostics/psram_free.yaml@main
```
...
- ESP32-C3/C6 have no PSRAM interface - this template is limited to esp32/s2/s3. Requires the psram: component (core/psram.yaml) configured AND PSRAM hardware present, otherwise the sensor reports 0.
