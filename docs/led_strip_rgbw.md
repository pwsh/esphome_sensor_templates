...
- DEPRECATION (2026.8): rgb_order + is_rgbw are deprecated in favor of channel_colors (e.g. GRBW) and will be REMOVED in 2027.3.0. This file keeps the old keys on purpose - channel_colors only exists on 2026.8.0+ (verified: 2026.7.4 rejects it) and switching now would break every earlier install. Until then 2026.8 prints a harmless warning. A ready-to-enable replacement ships commented out next to the rgb_order line: on 2026.8.0+, delete rgb_order and is_rgbw and uncomment channel_colors (the var value with W appended, e.g. GRB -> GRBW).
