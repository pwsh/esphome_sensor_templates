...
- Home Assistant 2026.6 changed the proxy's default scan mode to Auto (passive listening with brief active bursts, ~95% battery-device savings vs always-Active); devices previously set to Active were auto-migrated. ESPHome 2026.7 also fixed the 'connected but no advertisements' condition after HA restarts (a newer HA subscriber now takes over a stale slot). Both are runtime/HA-side - no config change here.
