...
- Effects are compiled in but cost nothing until activated from HA - an idle effect list adds only flash/RAM, no runtime work, so the curated list below is free to keep. The random effects drive the RGB channels; with st_rgbw_color_interlock left at "true" that means the white channel stays off while they run. The pulse effects only modulate brightness, so they breathe whatever color/white mix is currently set.
